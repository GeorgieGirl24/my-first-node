...
app.post('/api/v1/footnotes', (request, response) => {
  ...
  database('footnote').insert(footnote, 'id')
  .then(footnote => {
    response.status(201).json({ id: footnote[0], note: footnote[1] })
  })
  .catch(error => {
    response.status(500).json({ error });
  });
...
app.get('/api/v1/papers/:id/footnote', (request, response) => {
  ...
  .then(footnotes => {
    if (footnotes.length) {
      ...
      response.status(400).json({
        error: 'Could not find footnotes with paper_id ${request.params.id}'
      });
    }
  })
  .catch(error => {
